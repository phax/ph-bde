...
/**
 * BDE document type map. Provides sanity methods for accessing BDE document
 * types based on different information items.
 *
 * @author Philip Helger
 */
@Immutable
public final class BDEDocumentTypes
{
  /** Maps local names to document types */
  private static final ICommonsMap <String, EBDEDocumentType> s_aLocalName2DocType = new CommonsHashMap<> ();

  static
  {
    // Register all SBDH document types
    for (final EBDEDocumentType eDocType : EBDEDocumentType.values ())
    {
  ...
  private static final BDEDocumentTypes s_aInstance = new BDEDocumentTypes ();

  private BDEDocumentTypes ()
  {}
   ...
   */
  @Nullable
  public static EBDEDocumentType getDocumentTypeOfLocalName (@Nullable final String sLocalName)
  {
  ...
  {
    final EBDEDocumentType eDocType = getDocumentTypeOfLocalName (sLocalName);
   ...
   */
  @Nullable
  public static EBDEDocumentType getDocumentTypeOfImplementationClass (@Nullable final Class <?> aImplClass)
  {
    if (aImplClass == null)
      return null;
    return ArrayHelper.findFirst (EBDEDocumentType.values (),
  ...
  public static Schema getSchemaOfImplementationClass (@Nullable final Class <?> aImplClass,
                                                       @Nullable final ClassLoader aClassLoader)
  {
    final EBDEDocumentType eDocType = getDocumentTypeOfImplementationClass (aImplClass);
    return eDocType == null ? null : eDocType.getSchema (aClassLoader);
  }
}
